...
public class PulsarTenantSyncTest {

    @Test
    public void testSyncTenant() throws Exception {
        ...
        srcServer.start();
        dstServer.start();
        List<String> clusters = srcServer.createPulsarAdmin().clusters().getClusters();
        TenantInfo tenantInfo = TenantInfo.builder().allowedClusters(new HashSet<>(clusters)).build();
        srcServer.createPulsarAdmin().tenants().createTenant("test-tenant", tenantInfo);
        ...
        Awaitility.await().atMost(10, TimeUnit.SECONDS).until(() -> {
            List<String> tenants = dstPulsarAdmin.tenants().getTenants();
            return tenants.contains("test-tenant");
        });
        pulsarSync.close();
        srcServer.close();
        dstServer.close();
